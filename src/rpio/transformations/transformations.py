...
from robotransform import MapleKStore, write_output, generate_messages_aadl, generate_logical_aadl
        ...
        if swcExists:
            with open(join(swcFolder, process.name + ".py"), "r") as swcFile:
                ...
                for i in range(0, len(process.threads), 1):
                    content = replaceCustomCode(content, tag="code_" + process.threads[i].name,
                                                replacement=cc_thread_code[i])
...
def robochart2aadlmessages(store: MapleKStore, output_filepath=Path("output/generated/messages/messages.aadl")):
    output_filepath.parent.mkdir(parents=True, exist_ok=True)
    write_output(generate_messages_aadl(store), output_filepath)


def robochart2logical(store: MapleKStore, output_filepath=Path("output/generated/LogicalArchitecture/logical.aadl")):
    output_filepath.parent.mkdir(parents=True, exist_ok=True)
    write_output(generate_logical_aadl(store), output_filepath)
...
def update_robosapiensIO_ini(system=None, package="", prefix="", path="output/generated/docker"):
    ...
    with open(join(path, "robosapiensIO.ini"), 'w') as f:
        f.write(template.render(system=system, package=package, prefix=prefix, timestamp=formatted_timestamp.__str__(),
                                managingSystem=managingSystem, managedSystem=managedSystem))
